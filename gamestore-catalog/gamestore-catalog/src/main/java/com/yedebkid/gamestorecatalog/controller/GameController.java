...
import com.yedebkid.gamestorecatalog.viewModel.GameViewModel;
...
public class GameController {
    ...
    public GameViewModel createGame(@RequestBody @Valid GameViewModel gameViewModel) {
        gameViewModel = gameStoreCatalogServiceLayer.createGame(gameViewModel);
    ...
    public GameViewModel getGameInfo(@PathVariable("id") long gameId) {
        GameViewModel gameViewModel = gameStoreCatalogServiceLayer.getGame(gameId);
    ...
    public void updateGame(@RequestBody @Valid GameViewModel gameViewModel) {
        gameStoreCatalogServiceLayer.updateGame(gameViewModel);
    }

    @DeleteMapping("/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void deleteGame(@PathVariable("id") int gameId) {
        gameStoreCatalogServiceLayer.deleteGame(gameId);
    }

    @GetMapping("/title/{title}")
    @ResponseStatus(HttpStatus.OK)
    public List<GameViewModel> getGamesByTitle(@PathVariable("title") String title) {
        List<GameViewModel> gamesByTitle = gameStoreCatalogServiceLayer.getGameByTitle(title);
    ...
    public List<GameViewModel> getGamesByEsrbRating(@PathVariable("esrb") String esrb) {
        List<GameViewModel> gamesByEsrbRating = gameStoreCatalogServiceLayer.getGameByEsrb(esrb);
    ...
    public List<GameViewModel> getGamesByStudio(@PathVariable("studio") String studio) {
        List<GameViewModel> gamesByStudio = gameStoreCatalogServiceLayer.getGameByStudio(studio);
    ...
    public List<GameViewModel> getAllGames() {
        List<GameViewModel> games = gameStoreCatalogServiceLayer.getAllGames();
